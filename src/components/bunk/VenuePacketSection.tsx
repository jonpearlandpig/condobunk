...
const CATEGORY_LABELS: Record<VenueDocCategory, string> = {
  tech_packet: "Tech Packet",
  production_book: "Production Book",
  rigging_guide: "Rigging Guide",
  venue_map: "Venue Map",
  power_sheet: "Power Sheet",
...
export default function VenuePacketSection({ showAdvanceId, onAnalysisComplete }: Props) {
  ...
  const [selectedCategory, setSelectedCategory] = useState<VenueDocCategory>("tech_packet");

  const { data: docs, isLoading } = useQuery({
    queryKey: ["advance-venue-docs", showAdvanceId],
    queryFn: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (docId: string) => {
      ...
      const { error } = await supabase.from("advance_venue_docs").delete().eq("id", docId) as any;
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["advance-venue-docs", showAdvanceId] });
    ...
    <Card>
      <CardHeader className="py-3 px-4">
        <div className="flex items-center justify-between">
          <CardTitle className="text-xs font-mono tracking-wider text-muted-foreground/60">VENUE PACKETS</CardTitle>
      ...
      <CardContent className="px-4 pb-3 space-y-3">
        ...
        <div className="flex items-center gap-2">
          <Select value={selectedCategory} onValueChange={(v) => setSelectedCategory(v as VenueDocCategory)}>
        ...
        {isLoading ? (
          ...
          <p className="text-xs text-muted-foreground text-center py-3">
            No venue packets uploaded yet. Upload a tech packet, production book, or rigging guide to get started.
          </p>
        ) : (
